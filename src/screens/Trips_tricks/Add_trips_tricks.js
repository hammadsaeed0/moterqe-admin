...
const Add_trips_tricks = ({
    ...
    const bannerSubmit = async (values) => {
        if (!selectedImage) {
            ...
            toast.error("select your title!");
        } else if (values.vehicleId.value.length === 0) {
            toast.error("select your vehicle!");
        } else if (values.category.value.length === 0) {
            toast.error("select your category!");
        } else if (values.url.value.length === 0) {
            ...
            await axios
                .post(`https://autoproapp-1537b3ac9acb.herokuapp.com/tips/create`, params)
                .then((res) => {
                    ...
                    if (res.data.status === "success") {
                        ...
                        setLoading(false);
                        axios
                            .get(`https://autoproapp-1537b3ac9acb.herokuapp.com/tips/getAll`)
                            .then((res) => {
                                console.log(res.data);
    ...
    useEffect(() => {
        axios
            .get(`https://autoproapp-1537b3ac9acb.herokuapp.com/admin/vehicle/getAll`)
            .then((res) => {
                console.log(res.data);
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal}>
                ...
                <div className="">
                    <div className=" p-3 flex justify-between items-center">
                        <div></div>
                        <h1 className="capitalize h4 font-semibold">Add Trips & tricks</h1>
                        ...
                        >
                            <div className=" flex gap-5 flex-wrap">
                                <div className=" w-[100%]">
                                    ...
                                    <select name="vehicleId" className="border outline-none bg-lightGray  p-2.5 text-black placeholder:text-primary rounded-md   w-full  py-3">
                                        <option>--select vehicle--</option>
                                        {car?.map((item, index) => {
                                            return (
                                                <option value={item?._id}>{item?.carId?.title}</option>
                                            )
                                        })}
                                ...
                                <div className=" w-[100%]">
                                    ...
                                    <select name="category" className="border outline-none bg-lightGray  p-2.5 text-black placeholder:text-primary rounded-md   w-full  py-3">
...
export default Add_trips_tricks;
